...
\begin{gather}
    \delta = \Delta s = \frac{\Delta \varphi}{2\pi} \cdot \lambda
\end{gather}
\begin{center}
    \begin{tabular}{rl}
    ...
    \end{tabular}
\end{center}
Damit entspricht ein Gangunterschied von ganzzahligen Vielfachen der Wellenlänge ($\delta = k \cdot \lambda$) konstruktiver Interferenz, während ein ungeradzahliges Vielfaches der halben Wellenlänge ($\delta = (k + 0.5) \cdot \lambda$) zu destruktiver Interferenz führt.

Die Einheit des Gangunterschieds ist
\begin{center}
    $[\delta] = \si{\meter}$
\end{center}
...
Die Wellenlänge einer stehenden Welle kann durch Messung des Abstands benachbarter Knoten bestimmt werden: $\lambda = 2 \cdot d_\mathrm{knot}$
...
Auch in Rohren mit offenen oder geschlossenen Enden kommt es zu stehenden Wellen. Mit Knoten und Bäuchen verhält es sich ähnlich wie bei der schwingenden Saite. Hierbei muss unbedingt zwischen Schnellebäuchen und Druckbäuchen unterschieden werden. Platziert man Korkstaub in einer Röhre mit offenen Enden (Kundt'sches Rohr) und lässt das System mit seiner Grundfrequenz schwingen, entstehen an den Rändern Schnellebäuche bzw. Druckknoten. Am offenen Ende ist der Druck nämlich immer gleich dem konstanten Außendruck. Es kann sich kein Überdruck aufbauen. Deshalb ist dort ein Druckknoten. In der Mitte entsteht ein Schnelleknoten bzw. Druckbauch.

Insgesamt sind die Bedingungen für die \hyperref[sec:eigenschwingungen_mit_festen_und_losen_Enden]{Eigenschwingungen mit festen und losen Enden} zu beachten.

Werden an einer Wasseroberfläche von zwei gleichphasigen Erregern Wellen erzeugt, überlagern sich diese zu einem Interferenzmuster. Am Punkt genau zwischen den beiden Erregern ist der Weg von beiden Erregern aus gleich lang, sodass der Gangunterschied $\delta = 0$ beträgt und ein Wellenbauch entsteht (konstruktive Interferenz).
Allgemein treten Wellenbäuche dort auf, wo der Gangunterschied, also die Differenz der Abstände von den beiden Erregern zu dem betrachteten Punkt, ein ganzzahliges Vielfaches der Wellenlänge ist: $\delta = k \cdot \lambda \ (k \in \mathbb{N}_0)$
Wellenknoten (destruktive Interferenz) entstehen dort, wo der Gangunterschied ein ungeradzahliges Vielfaches der halben Wellenlänge ist: $\delta = \left(k + \tfrac{1}{2}\right) \cdot \lambda \ (k \in \mathbb{N}_0)$
Auf der Verbindungsstrecke der Erreger beträgt der Abstand zwischen benachbarten Knoten oder Bäuchen jeweils $\tfrac{\lambda}{2}$.
...
Bei konstruktiver Interferenz verstärken sich die Wellen, wenn ihre Auslenkungen in die gleiche Richtung zeigen ($\delta \approx k \cdot \lambda$). Bei destruktiver Interferenz schwächen sich die Wellen ab oder löschen sich aus, wenn ihre Auslenkungen entgegenwirken ($\delta \approx (k + 0.5) \cdot \lambda$).
